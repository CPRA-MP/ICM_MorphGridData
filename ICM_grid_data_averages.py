...
grid_bed_z[g]
grid_land_z[g]
100.0*grid_pct_land[g]
100.0*grid_pct_land_wetl[g]
100.0*grid_pct_water[g]
100.0*grid_pct_edge[g]
...
with open(grid_data_file,mode='w') as gdaf:  
    ...
    for g in grid_bed_z.keys():
        gdaf.write('%d,%0.4f,%0.4f,%0.2f,%0.2f,%0.2f\n' % (g,grid_bed_z[g],grid_land_z[g],100.0*grid_pct_land[g],100.0*grid_pct_land_wetl[g],100.0*grid_pct_water[g]) )
    
with open(grid_pct_edge_file,mode='w') as gdef:  
    gdef.write('GRID,PERCENT_EDGE_0-100\n')
    for g in grid_pct_edge.keys():
        gdef.write('%d,%0.4f' % (g,100.0*grid_pct_edge[g]) )
